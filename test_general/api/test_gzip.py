...
def get_gzipped_response(client: TestClient, original_file: str, options: dict):
...
def compare_responses(response1: httpx.Response, response2: httpx.Response, output_format: str):
    if output_format == "application/json":
        diff = DeepDiff(t1=response1.json(), t2=response2.json(), exclude_regex_paths=r"root\[\d+\]\['metadata'\]\['filename'\]")
        assert len(diff) == 0
    else:
        df1 = pd.read_csv(io.StringIO(response1.text))
        df2 = pd.read_csv(io.StringIO(response2.text))
        diff = DeepDiff(t1=df1.to_dict(), t2=df2.to_dict(), exclude_regex_paths=r"root\['filename'\]\[\d+\]")
        assert len(diff) == 0
...
def test_single_gzipped_file_is_parsed_same_as_original_explicit_gz_type(output_format: str, example_filename: str, uncompressed_content_type: str):
    """
    ...
    gz_options = {
        "gz_uncompressed_content_type": uncompressed_content_type if uncompressed_content_type else None,
        "output_format": output_format
    }

    response1 = get_gzipped_response(client, str(original_file), gz_options)
    response2 = call_api(client, str(original_file), uncompressed_content_type, {"output_format": output_format})
