...
# check that text_as_html appears in the metadata in the hi_res case
@pytest.mark.parametrize(
    "example_filename, content_type",
    [
        ("layout-parser-paper.pdf", "application/pdf"),
...
def test_table_to_html_string(example_filename, content_type):
    client = TestClient(app)
    test_file = Path("sample-docs") / example_filename
    response = client.post(
        MAIN_API_ROUTE,
    ...
    assert len(response.json()) > 0
    for i in response.json():
        assert "text_as_html" in i["metadata"].keys()


# check that text_as_html does not appear in the metadata
# if strategy=hi_res and pdf_infer_table_structure=false
@pytest.mark.parametrize(
    "example_filename, content_type",
    [
        ("layout-parser-paper.pdf", "application/pdf"),
...
def test_text_as_html(example_filename, content_type):
    client = TestClient(app)
    test_file = Path("sample-docs") / example_filename
    response = client.post(
        MAIN_API_ROUTE,
        files=[("files", (str(test_file), open(test_file, "rb"), content_type))],
        data={"strategy": "hi_res", "pdf_infer_table_structure": False},
    )
    assert response.status_code == 200
    assert len(response.json()) > 0
    for i in response.json():
        assert "text_as_html" not in i["metadata"].keys()
